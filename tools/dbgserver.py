...
class DbgBridge:
	...
	def cmd_read_regs(self):
		...
		regs_hex = b''
		regs_hex += b''.join(binascii.hexlify(struct.pack("<Q", reg)) for reg in gregs)
	...
	def cmd_write_regs(self, arg):
		...
		regs_bytes_le_list = [regs_bytes_le[i:i+8] for i in range(0, len(regs_bytes_le), 8)]
		# remap little endian data into host integers
		# unpack: bytes object -> (integer, ...) tuple
		regs_ints_list = map(lambda chunk: struct.unpack("<Q", chunk)[0], regs_bytes_le_list)
	...
	def cmd_read_mem(self, args):
		...
		for offset in range(0, read_length, 4):
				data_le += struct.pack("<I", struct.unpack(">I", data_be[offset:offset+4])[0])
	...
	def cmd_write_mem(self, args):
		...
		final_le = data_le[:start_byte] + override_le + data_le[(start_byte+length):]
